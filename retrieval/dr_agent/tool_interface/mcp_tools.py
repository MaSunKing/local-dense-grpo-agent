...
class PubMedSearchTool(MCPSearchTool):
    """Search PubMed through the DR Tulu MCP backend."""
    ...
    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        raw = dict(tool_call_info.parameters)
        raw["query"] = tool_call_info.content
        params = normalize_medical_tool_arguments("pubmed_search", raw)
        params.setdefault("limit", min(self.number_documents_to_search, 10))
        params.setdefault("offset", 0)
        return params

    def extract_documents(self, raw_output: Dict[str, Any]) -> List[Document]:
        documents = []
        for item in raw_output.get("data", []):
            if not isinstance(item, dict):
                continue
            pmid = str(item.get("pmid") or item.get("paperId") or "").strip()
            abstract = str(item.get("abstract") or item.get("text") or "").strip()
            metadata = []
            if pmid:
                metadata.append(f"PMID: {pmid}")
            if item.get("journal") or item.get("venue"):
                metadata.append(f"Journal: {item.get('journal') or item.get('venue')}")
            if item.get("year"):
            ...
            if abstract:
                metadata.append(f"Abstract: {abstract}")
            ...
            documents.append(
                Document(
                    id=str(item.get("source_id") or (f"PMID:{pmid}" if pmid else "")),
                    title=title,
                    snippet="\n".join(metadata),
...
class MedicalDocumentBrowseTool(MCPSearchTool):
    """Retrieve bounded, query-relevant passages from PubMed/PMC documents."""
    ...
    def preprocess_input(
        ...
        if isinstance(tool_input, dict):
            source_id = str(tool_input.get("source_id", "")).strip()
            if not source_id:
                return None
            return ToolCallInfo(
                content=source_id,
                parameters={k: v for k, v in tool_input.items() if k != "source_id"},
                start_pos=0,
                end_pos=len(source_id),
            )
        return super().preprocess_input(tool_input)

    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        raw = dict(tool_call_info.parameters)
        raw["source_id"] = tool_call_info.content
        params = normalize_medical_tool_arguments("browse_document", raw)
        query = str(params.get("query", "")).strip()
        if not query:
            raise ValueError("browse_document requires a focused query parameter")
        params.setdefault("top_k", 2)
        params.setdefault("max_chars", 3000)
        params.setdefault("max_output_tokens", 800)
        return params
    ...
    def extract_documents(self, raw_output: Dict[str, Any]) -> List[Document]:
        ...
        for item in raw_output.get("data", []):
            if not isinstance(item, dict):
                continue
            chunk_id = str(item.get("source_id") or "")
            heading = str(item.get("heading") or "Section")
            content_level = str(item.get("content_level") or "")
...
class MedicalWebSearchTool(MCPSearchTool):
    """Search server-managed authoritative medical domains through Serper."""
    ...
    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        raw = dict(tool_call_info.parameters)
        raw["query"] = tool_call_info.content[:SERPER_MAX_QUERY_LENGTH]
        params = normalize_medical_tool_arguments("medical_web_search", raw)
        params.setdefault("source_types", "guideline")
        params.setdefault("limit", min(self.number_documents_to_search, 10))
...
class MedicalWebBrowseTool(MCPSearchTool):
    """Read focused, bounded evidence from an authoritative medical webpage."""

    def get_mcp_tool_name(self) -> str:
        return "browse_medical_webpage"
    ...
    def preprocess_input(
        ...
        if isinstance(tool_input, dict):
            source_id = str(
                tool_input.get("source_id") or tool_input.get("url") or ""
            ).strip()
            if not source_id:
                return None
            return ToolCallInfo(
    ...
    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        raw = dict(tool_call_info.parameters)
        raw["source_id"] = tool_call_info.content
        params = normalize_medical_tool_arguments("browse_webpage", raw)
        query = str(params.get("query", "")).strip()
        if not query:
            raise ValueError("browse_webpage requires a focused query parameter")
        params["url"] = params.pop("source_id")
        params.setdefault("top_k", 2)
        params.setdefault("max_chars", 3000)
        params.setdefault("max_output_tokens", 800)
        return params
