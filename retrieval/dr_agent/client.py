...
from .context_manager import compact_medical_context
from .medical_runtime import MedicalRuntimeController
...
class GenerateWithToolsOutput(BaseModel):
    ...
    medical_runtime_events: List[Dict[str, Any]] = Field(default_factory=list)
    medical_answer_accepted: Optional[bool] = None
...
class LLMToolClient:
    ...
    @staticmethod
    def _infer_user_question(
        prompt_or_messages: Union[str, List[Dict[str, str]]]
    ) -> str:
        if isinstance(prompt_or_messages, list):
            for message in reversed(prompt_or_messages):
                if message.get("role") == "user":
                    return str(message.get("content") or "")
        return str(prompt_or_messages or "")
    ...
    async def generate_with_tools(
        ...
        context_soft_limit_tokens: Optional[int] = None,
        final_answer_reserve_tokens: int = 2048,
        medical_runtime_enabled: bool = False,
        medical_question: Optional[str] = None,
        medical_route_class: Optional[str] = None,
        medical_claim_support_validator: Optional[Callable[..., Union[bool, str]]] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            context_soft_limit_tokens: Optional working-context target. The hard
                total-sequence limit remains ``max_tokens``.
            final_answer_reserve_tokens: Tokens reserved for final synthesis when
                the working context approaches the hard limit.
            medical_runtime_enabled: Apply the shared medical duplicate, evidence,
                and final-answer gates. Currently supported by parser-mode vLLM.
            ...
            if tool_calling_mode == "native":
                ...
                if self.is_commercial_api_model:
                    return await self._generate_with_tools_commercial_api(
                        ...
                        medical_runtime_enabled,
                        medical_question,
                        medical_route_class,
                        medical_claim_support_validator,
    ...
    async def _generate_with_tools_commercial_api(
        ...
        context_soft_limit_tokens: Optional[int],
        final_answer_reserve_tokens: int,
        medical_runtime_enabled: bool,
        medical_question: Optional[str],
        medical_route_class: Optional[str],
        medical_claim_support_validator: Optional[Callable[..., Union[bool, str]]],
        on_step_callback: Optional[Callable[[str, List[ToolOutput]], None]] = None,
        **kwargs,
    ) -> GenerateWithToolsOutput:
    ...
    async def _generate_with_tools_vllm(
        ...
        **kwargs,
        ...
        medical_runtime = (
            MedicalRuntimeController(
                question=medical_question or self._infer_user_question(prompt_or_messages),
                route_class=medical_route_class,
                max_tool_calls=max_tool_calls,
                claim_support_validator=medical_claim_support_validator,
            )
            if medical_runtime_enabled
            else None
        )
        context_peak_tokens = self._count_tokens(current_context)
        context_compactions = 0
        stopped_by_budget = False
        budget_stop_reason: Optional[str] = None
        budget_warning_sent = False
        medical_answer_accepted: Optional[bool] = None
        medical_terminal_reason: Optional[str] = None
        ...
        while True:
            ...
            if context_soft_limit_tokens is not None:
                managed = compact_medical_context(
                    current_context,
                    count_tokens=self._count_tokens,
                    soft_limit_tokens=context_soft_limit_tokens,
                )
                current_context = managed.text
                context_compactions += managed.compacted_blocks
                context_peak_tokens = max(context_peak_tokens, managed.tokens)
                if managed.tokens >= base_max_tokens - final_answer_reserve_tokens:
                    stopped_by_budget = True
                    budget_stop_reason = "context_budget"
            ...
            dynamic_max_tokens = min(
                dynamic_max_tokens,
                max(256, final_answer_reserve_tokens),
            )
            # print(
            ...
            if generation_prefix and iteration == 1:
                ...
                audit_fragments.append(generation_prefix)
            ...
            audit_fragments.append(response)
            ...
            if not tool_match:
                if medical_runtime is not None and "</answer>" in response:
                    answer_start = response.rfind("<answer>")
                    proposed_answer = response[answer_start:] if answer_start >= 0 else response
                    verdict = medical_runtime.validate_answer(proposed_answer)
                    medical_answer_accepted = verdict.accepted
                    if not verdict.accepted and medical_runtime.finalizer_rejections < 2:
                        validation_message = (
                            "\n<validation_error>The proposed answer was not sufficiently grounded "
                            "in the opened evidence. Review the question and observations, then "
                            "replan within the remaining budget without inventing evidence."
                            "</validation_error>\n<runtime_state>"
                            + json.dumps(medical_runtime.public_state(), ensure_ascii=False, separators=(",", ":"))
                            + "</runtime_state>\n"
                        )
                        current_context += validation_message
                        audit_fragments.append(validation_message)
                        continue
                    if not verdict.accepted:
                        medical_terminal_reason = "finalizer_rejected"
                # No tool calls found, or the final answer passed the shared gate.
                if verbose:
                    print("No tool calls found, finishing.")
                break
            ...
            if stopped_by_budget:
                ...
                budget_message = (
                    "\n<tool_output>Context budget reached. Do not call another tool; "
                    "produce the best evidence-bounded final answer and explicitly state "
                    "any unresolved evidence gap.</tool_output>\n"
                )
                current_context += budget_message
                audit_fragments.append(budget_message)
                continue
            ...
            if tool_call_count >= max_tool_calls:
                if verbose:
                    print(
                        f"Reached maximum tool calls ({max_tool_calls}); stopping tool execution."
                    )

                stopped_by_budget = True
                budget_stop_reason = "tool_budget"
                break
                ...
                parsed_call = tool.parse_call(
                    generation_prefix + response
                    if generation_prefix and iteration == 1
                    else response
                )
                blocked_output = None
                if medical_runtime is not None and parsed_call is not None:
                    arguments = medical_runtime.arguments_from_parsed_call(
                        tool.name, parsed_call.content, parsed_call.parameters
                    )
                    blocked_output = medical_runtime.prepare_action(tool.name, arguments)
                if blocked_output is not None:
                    tool_output = tool._create_error_output(
                        error_msg=str(blocked_output["error"]),
                        call_id="",
                        runtime=0,
                        output=str(blocked_output["error"]),
                        raw_output=blocked_output,
                    )
                    medical_runtime.record_blocked_output(tool.name, blocked_output)
                elif medical_runtime is not None and parsed_call is not None:
                    # Keep the model-visible call and duplicate-policy arguments
                    # unchanged. The private original-question anchor exists only
                    # in the MCP execution payload.
                    execution_arguments = medical_runtime.execution_arguments(
                        tool.name, arguments
                    )
                    tool_output = await tool(execution_arguments)
                elif generation_prefix and iteration == 1:
                    tool_output = await tool(generation_prefix + response)
                else:
                    tool_output = await tool(response)
                if medical_runtime is not None and blocked_output is None:
                    annotated_output = medical_runtime.record_output(tool.name, tool_output)
                    tool_output.raw_output = annotated_output
                tool_call_count += 1
                ...
                if include_tool_results and (tool_output.called or blocked_output is not None):
                    # Append tool result to context
                    result_formatted = tool.format_result(tool_output)
                    current_context += result_formatted
                    audit_fragments.append(result_formatted)
                    if medical_runtime is not None:
                        medical_runtime.confirm_last_output_delivery(tool.name)
                        runtime_message = (
                            "\n<runtime_state>"
                            + json.dumps(medical_runtime.public_state(), ensure_ascii=False, separators=(",", ":"))
                            + "</runtime_state>\n"
                        )
                        current_context += runtime_message
                        audit_fragments.append(runtime_message)
                    # TODO: maybe we should add a new line after the tool result

                    if verbose:
                        print(f"Tool output: {tool_output.output}")

                if tool_call_count >= max_tool_calls and not budget_warning_sent:
                    stopped_by_budget = True
                    budget_stop_reason = "tool_budget"
                    budget_warning_sent = True
                    budget_message = (
                        "\n<tool_output>Agent tool-call budget reached. Do not call "
                        "another tool; produce the best evidence-bounded final answer "
                        "and explicitly state any unresolved evidence gap.</tool_output>\n"
                    )
                    current_context += budget_message
                    audit_fragments.append(budget_message)
            ...
            context_peak_tokens = max(context_peak_tokens, final_token_count)
            if context_soft_limit_tokens is not None:
                managed = compact_medical_context(
                    current_context,
                    count_tokens=self._count_tokens,
                ...
                current_context = managed.text
                context_compactions += managed.compacted_blocks
                final_token_count = managed.tokens
                if final_token_count >= base_max_tokens - final_answer_reserve_tokens:
                    stopped_by_budget = True
                    budget_stop_reason = "context_budget"
            if final_token_count >= base_max_tokens:
                if verbose:
        ...
        return GenerateWithToolsOutput(
            tool_calls=tool_calls,
            generated_text="".join(audit_fragments),
            total_tokens=self._count_tokens(current_context),
            tool_call_count=tool_call_count,
            context_peak_tokens=context_peak_tokens,
            context_compactions=context_compactions,
            stopped_by_budget=stopped_by_budget,
            duplicate_blocks=(medical_runtime.duplicate_blocks if medical_runtime else 0),
            finalizer_rejections=(medical_runtime.finalizer_rejections if medical_runtime else 0),
            medical_runtime_events=(medical_runtime.events if medical_runtime else []),
            medical_answer_accepted=medical_answer_accepted,
            stopped_reason=(
                medical_terminal_reason
                if medical_terminal_reason
                else budget_stop_reason
                if stopped_by_budget and budget_stop_reason
                else (
                    "max_tokens"
